...
const learningData = [
  { title: "AI / ML", desc: "Building foundations in artificial intelligence and machine learning.", status: "LEARNING", tone: "cyan" },
  { title: "Mathematics", desc: "Learning the math needed for AI/ML, including linear algebra.", status: "BUILDING FOUNDATIONS", tone: "yellow" },
  { title: "DSA", desc: "Learning data structures, algorithms, complexity, and problem-solving.", status: "PRACTICING", tone: "cyan" },
  { title: "JavaScript / Web", desc: "Building a stronger foundation in JavaScript and web development.", status: "LEARNING", tone: "cyan" },
  { title: "Quantum Computing", desc: "Exploring the concepts and mathematics behind quantum computing.", status: "EXPLORING", tone: "pink" },
  { title: "Research", desc: "Exploring AI/ML research and building the foundations to move toward it.", status: "EXPLORING", tone: "pink" },
];
...
const Learning = () => (
  ...
  >
    ...
    <div className="mx-auto max-w-[1200px]">
      ...
      {/* math journal */}
      <div className="mt-16 grid gap-9 rounded-lg border-[3px] border-navy bg-paper p-8 shadow-retro journal-lines md:grid-cols-[1fr_1.2fr]">
        ...
        <ul className="flex flex-col gap-1">
          {mathCheckpoints.map((item, i) => (
            <li
              className="flex items-center gap-3 border border-navy/15 bg-white/80 px-3.5 py-2.5 transition-transform duration-150 hover:translate-x-1"
              key={item}
            >
              <span className="h-5 w-5 flex-shrink-0 border-[3px] border-navy bg-white shadow-[2px_2px_0_rgba(23,23,55,0.5)]" aria-hidden="true" />
              <span className="font-vt text-xl font-bold text-navy">
      ...
      <div className="mt-16">
        <div className="mb-9">
          ...
          <p className="mt-3 max-w-[560px] text-[16px] leading-relaxed text-white/55">
            Building my foundation step by step. Some stages are in progress, none
            are finished — that's the point.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-7 sm:grid-cols-3 lg:grid-cols-6">
          {journey.map((j, i) => (
            <div className="relative flex flex-col items-center gap-2 text-center" key={j.step}>
              ...
              <span className="font-vt text-sm text-white/50">{j.note}</span>
